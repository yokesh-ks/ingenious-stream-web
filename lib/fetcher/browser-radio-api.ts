...
export async function searchStations(query: string, limit: number = 50): Promise<RadioStation[]> {
	...
	try {
		const response = await fetch(
			`${API_BASE}/stations/byname/${encodeURIComponent(query)}?limit=${limit}`,
			{
				headers: {
					"User-Agent": "RadioHub/1.0",
				},
...
export async function getStationsByGenre(
	...
	try {
		const response = await fetch(
			`${API_BASE}/stations/bytag/${encodeURIComponent(genre)}?limit=${limit}`,
			{
				headers: {
					"User-Agent": "RadioHub/1.0",
				},
...
export async function getStationsByCountry(
	...
	try {
		const response = await fetch(
			`${API_BASE}/stations/bycountry/${encodeURIComponent(country)}?limit=${limit}`,
			{
				headers: {
					"User-Agent": "RadioHub/1.0",
				},
...
export async function getStationsByLanguage(
	...
	try {
		const response = await fetch(
			`${API_BASE}/stations/bylanguage/${encodeURIComponent(language)}?limit=${limit}`,
			{
				headers: {
					"User-Agent": "RadioHub/1.0",
				},
